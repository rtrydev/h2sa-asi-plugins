 ...
 *
 * For H2SA the target module is RenderD3D.dll (the active DrawDll): its
 * software vertex-transform/lighting path is dense x87 whose cost scales
 * with on-screen geometry, which is what drags the frame rate down in
 * crowded areas. The loader is module-agnostic — it applies whatever
 * <module>.x87 files are present, keyed by the module name in the header.
 */
#include <windows.h>
#include <stdio.h>
#include <stdint.h>
...
/* ---- diagnostics (used by H2SA_DIAG builds of helpers.S) ---- */
...
static int apply_patch(Pending *p, HMODULE mod)
{
    ...
    if (memcmp(h->magic, "H2SAX87P", 8) || h->version != 1) {
    ...
    /* Unpack gate: hitman2.exe ships packed — its .text is garbage until the
     * startup unpacker rewrites it, and hooking before then would be
     * overwritten (or corrupt the unpacker). When the patch carries original
     * entry bytes, only proceed once (nearly) every function entry matches;
     * otherwise report "not ready" so the poller retries.
     *
     ...
     * legitimately differ from the on-disk originals wherever a reloc lands
     * in the first 5 bytes, so the byte match can't be used there — such
     * modules (e.g. RenderD3D.dll) aren't packed anyway, so hook them all. */
...
static void init(HMODULE self)
{
    ...
    /* Always poll: a packed module (hitman2.exe) is "loaded" from the start
     * but only becomes hookable once its unpacker runs, which fires no loader
     * notification — the poller retries apply_patch until the entry bytes
     * match the recorded originals. */
